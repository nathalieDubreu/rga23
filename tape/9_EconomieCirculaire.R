...
scoreMarchesLocaux <- left_join(
  ...
  mutate(score = case_when(
    (is.na(ComCulturesLocal) | ComCulturesLocal == 1 | venteProduitsVegetaux == 0) & (is.na(ComElevageLocal) | ComElevageLocal == 1 | venteProduitsAnimaux == 0) ~ 0,
    (is.na(ComCulturesLocal) | ComCulturesLocal == 2 | venteProduitsVegetaux == 0) & (is.na(ComElevageLocal) | ComElevageLocal == 2 | venteProduitsAnimaux == 0) ~ 1,
    (is.na(ComCulturesLocal) | ComCulturesLocal == 3 | venteProduitsVegetaux == 0) & (is.na(ComElevageLocal) | ComElevageLocal == 3 | venteProduitsAnimaux == 0) ~ 2,
    (is.na(ComCulturesLocal) | ComCulturesLocal == 4 | venteProduitsVegetaux == 0) & (is.na(ComElevageLocal) | ComElevageLocal == 4 | venteProduitsAnimaux == 0) ~ 3,
    (is.na(ComCulturesLocal) | ComCulturesLocal == 5 | venteProduitsVegetaux == 0) & (is.na(ComElevageLocal) | ComElevageLocal == 5 | venteProduitsAnimaux == 0) ~ 4,
    TRUE ~ 5
  ))
...
restentAClasser <- scoreMarchesLocaux |>
  filter(score == 5) |>
...
# > 0 - Il n’existe aucun réseau de producteurs pour commercialiser la production agricole. Aucune relation avec les consommateurs. Les intermédiaires gèrent l’ensemble du processus de commercialsation.
# > 1 - Les réseaux existent mais ne fonctionnent pas correctement. Peu de relations avec les consommateurs. Les intermédiaires gèrent la plupart du processus de commercialisation.
# > 2 - Les réseaux existent et sont opérationnels, mais n’incluent pas les femmes. Il existe une relation directe avec les consommateurs. Les intermédiaires gèrent une partie du processus de commericalisation.
# > 3 - Les réseaux existent, sont opérationnels, et comprennent les femmes. Il existe une relation directe avec les consommateurs. Les intermédiaires gèrent une partie du processus de commercialisation.
# > 4 - Des réseaux bien établis et opérationnels existent avec une participation égale des femmes. Relation solide et stable avec les consommateurs. Pas d’intermédiaires.
#
