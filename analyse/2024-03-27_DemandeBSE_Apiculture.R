...
nombreTotalApiculteurs <- sum(rga23_prodAnimales$PresenceAnimaux__7 == 1, na.rm = TRUE)
# 212 apiculteurs

rga23_apiculteurs <- rga23_prodAnimales |>
  filter(PresenceAnimaux__7 == 1) |>
  mutate(
    CategorieNombreRuches = case_when(
...
comptagesApiculteurs <- rga23_apiculteurs |>
  summarise(
...
rga23_apiculture_comptagesParArchipel <- rga23_apiculteurs |>
  group_by(Archipel_1) |>
  summarise(
    NombreApiculteurs = sum(PresenceAnimaux__7, na.rm = TRUE),
    NombreRuches = sum(NbRuchesPourProduire, na.rm = TRUE),
...
rga23_apiculture_comptagesParCategorie <- rga23_apiculteurs |>
