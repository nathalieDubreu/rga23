...
## Zoom sur l'appartenance au champ des `r eligiblesRGA |> filter(substring(id_exploitation, 0, 1) != "C") |> count()` unités ayant répondu au questionnaire complet (en tant qu'exploitation agricole - et non en tant que coprahculteurs)

```{r, echo=FALSE}
kable(inner_join(rga23, idExploitantsDansLeChamp, by = c("interview__key")) |>
  summarise(
    `Ensemble` = n(),
    `Cultivateurs` = sum(CultureValideRGA == 1, na.rm = TRUE),
    `Eleveurs` = sum(ElevageValideRGA == 1, na.rm = TRUE)
  ))
```
```{r, echo=FALSE}
kable(inner_join(rga23, idExploitantsDansLeChamp, by = c("interview__key")) |>
