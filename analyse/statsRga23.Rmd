...
)) |> group_by(TpsTravailChefExpl) |> count())
```
...
)) |> group_by(TempsTravailCoExploit) |> count())
```
...
)) |> group_by(TempsTravailMOFamPerm) |> count())
```
...
- Dont `r round(rga23_champ |>
  ...
  mutate(tempsTotalTravailHeures = case_when(
    ...
    TRUE ~ DureeMOOccas
  ), tempsTravailHeures = tempsTotalTravailHeures / totalMAOccas) |> filter(tempsTravailHeures < 50) |> summarize(sum(totalMAOccas)) / rga23_champ |> summarize(sum(totalMAOccas, na.rm = TRUE))*100,0)`% pour moins de 50h sur l'année
