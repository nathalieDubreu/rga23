...
rga23_champ <- rga23A_valides |> filter(ValideRGA == 1 | CoprahValideRGA == 1) |> mutate(Archipel_1 = case_when(!is.na(ArchipelExploitation) ~ ArchipelExploitation, TRUE ~ Archipel))
...
rga23_champ_date <- rga23_champ |> mutate(age = 2023 - as.numeric(substring(DateNaissChefExpl,7,10))) |> select(interview__key, age, DateNaissChefExpl)
```
