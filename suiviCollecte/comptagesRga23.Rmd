...
nbEntretiensTermines <- exportRGA |> filter(interview__status == 120 | interview__status == 100) |> count()
```

## `r exportRGA |> count()` entretiens *(Taux de collecte = `r round(exportRGA |> count() / 7494,3)*100`%)*

- <font color = "Green">`r exportRGA |> filter(interview__status == 120) |> count()`</font> entretiens approuvés
...
**=> `r nbEntretiensTermines` entretiens sont donc terminés et/ou validés**

<br/>

## Statut de la collecte pour ces `r exportRGA |> filter(interview__status == 120 | interview__status == 100) |> count()` entretiens

```{r, echo=FALSE}

kable(exportRGA |>
  filter(interview__status == 100 | interview__status == 120) |>
  mutate(
...
**=> `r exportRGA_interroges |> filter(interview__status == 120 | interview__status == 100) |> count()` unités ont donc été visitées**
...
- <font color = "Green">`r exportRGA_interroges |> filter(eligibOrNot == "in") |> count()`</font> unités éligibles (`r round(exportRGA_interroges |> filter(eligibOrNot == "in") |> count() / exportRGA_interroges |> count(),3)*100`% des unités visitées)

- <font color = "Red">`r exportRGA_interroges |> filter(eligibOrNot == "out") |> count()`</font> unités NON éligibles
